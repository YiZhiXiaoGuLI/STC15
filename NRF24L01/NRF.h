...
日期：2013年8月26日
控制原理：
	基于2.4GHz的NRF24L01+的无线接收和发送一体代码
	...
	sbit	IRQ		=P3^3;
	引出函数有：
	初始化NRF24L01：init_NRF24L01(void);
	发送函数：send(char *str)
	接收函数：receive(char *Receive);

使用说明：直接引入24L01头文件
	发送：直接将发送的数据按位存放在数组中，然后调用send(char *str)发送
	接收：先声明一个数组，然后调用receive(char *Receive)函数接收
***********************/
