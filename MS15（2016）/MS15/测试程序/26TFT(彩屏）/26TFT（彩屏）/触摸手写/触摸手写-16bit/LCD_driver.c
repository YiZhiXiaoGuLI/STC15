...
//写汉字子函数
//**********************************************************************************
void ChineseChar(uint x,uint y,uint For_color,uint Bk_color ,char c)
{
   int e=0,i,j;
   int  ed;
   ...
   for(i=0;i<nbytes;i++)
   	{
     		...
     		for(j=0;j<e;j++)
     		{
	  			if((ed>>j)&0x01)
      		{ 
	   			LCD_Write_DATA(For_color>>8 , For_color  ); 
	   		}
     			else
    		  {
       			LCD_Write_DATA( Bk_color>>8 ,  Bk_color); 
	 		  }
	     }
     }
}
